...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (n?.value) { setName(n.value); setView("home"); } else setView("welcome");
        if (s?.value) setStars(+s.value||0);
  ...
  const saveName = async (v: string): Promise<void> => {
    ...
    if (!n) return;
    setName(n);
    try { await window.storage.set("cname",n); } catch {}
    setView("home");
  };
  ...
  const openGame = (g: GameContext | {type: string}, fromDay: boolean = false): void => { setGameCtx({...g as GameContext, fromDay}); setView("game"); };
  ...
  if (view==="day")     return dayCtx ? <DayView day={dayCtx} name={name} isDone={done.includes(dayCtx.id)}
    onGame={(g: GameContext | {type: string})=>openGame(g,true)} onComplete={()=>{ markDone(dayCtx.id); goHome(); }} onBack={goHome}/> : null;

  return (
    <div style={{position:"relative"}}>
      <Home name={name} stars={stars} done={done} onDay={openDay} onGame={(g: GameContext | {type: string})=>openGame(g,false)}/>
  ...
  return (
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:"linear-gradient(135deg,#fbbf24 0%,#f472b6 50%,#a78bfa 100%)",padding:24,textAlign:"center"}}>
      ...
      <div style={{background:"white",borderRadius:28,padding:32,boxShadow:"0 24px 64px rgba(0,0,0,.2)",maxWidth:380,width:"100%"}}>
        ...
        <p style={{fontSize:20,fontWeight:800,color:"#7c3aed",marginBottom:16}}>What's your name?</p>
        <input value={v} onChange={(e: React.ChangeEvent<HTMLInputElement>)=>setV(e.target.value)} onKeyDown={(e: React.KeyboardEvent<HTMLInputElement>)=>e.key==="Enter"&&onSave(v)}
          placeholder="Type your name…" maxLength={12}
          style={{width:"100%",padding:"14px 18px",fontSize:24,borderRadius:14,border:"3px solid #a78bfa",outline:"none",fontWeight:800,textAlign:"center",color:"#4c1d95",boxSizing:"border-box"}}/>
...
// ── Loader ────────────────────────────────────────────────────────
function Loader() {
  return <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"linear-gradient(135deg,#a78bfa,#f472b6)"}}>
    <div style={{fontSize:80,animation:"spin 1.5s linear infinite"}}>🌟</div>
  </div>;
}

// ── Home ──────────────────────────────────────────────────────────
function Home({name,stars,done,onDay,onGame}: {name: string; stars: number; done: number[]; onDay: (d: DayConfig) => void; onGame: (g: GameContext | {type: string}) => void}) {
  ...
  const resetProgress = async (): Promise<void> => {
    try {
      await window.storage.set("cstars","0");
      await window.storage.set("cdone","[]");
    } catch {}
    window.location.reload();
  };
  ...
  if(editing) return (
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:"linear-gradient(135deg,#fbbf24 0%,#f472b6 50%,#a78bfa 100%)",padding:24,textAlign:"center"}}>
      ...
      <div style={{background:"white",borderRadius:28,padding:32,boxShadow:"0 24px 64px rgba(0,0,0,.2)",maxWidth:380,width:"100%"}}>
        <p style={{fontSize:16,fontWeight:700,color:"#7c3aed",marginBottom:12}}>Current: <strong>{name}</strong></p>
        <input value={newName} onChange={(e: React.ChangeEvent<HTMLInputElement>)=>setNewName(e.target.value)} onKeyDown={(e: React.KeyboardEvent<HTMLInputElement>)=>e.key==="Enter"&&saveName(newName)}
          placeholder="Type new name…" maxLength={12}
          style={{width:"100%",padding:"14px 18px",fontSize:20,borderRadius:14,border:"3px solid #a78bfa",outline:"none",fontWeight:800,textAlign:"center",color:"#4c1d95",boxSizing:"border-box",marginBottom:14}}/>
        <div style={{display:"flex",gap:10}}>
  ...
  return (
    <div style={{minHeight:"100vh",background:"#f0f4ff",paddingBottom:40}}>
      <div style={{background:"linear-gradient(135deg,#7c3aed,#ec4899)",padding:"28px 20px",borderRadius:"0 0 36px 36px",textAlign:"center",boxShadow:"0 6px 24px rgba(124,58,237,.35)"}}>
        <div style={{fontSize:52}}>🎒</div>
        <h1 style={{fontSize:30,fontWeight:900,color:"white",margin:"8px 0 2px"}}>Hello, {name}! 👋</h1>
        ...
        <div style={{display:"inline-flex",gap:10,background:"rgba(255,255,255,.2)",borderRadius:20,padding:"8px 22px",alignItems:"center"}}>
          <span style={{fontSize:26}}>⭐</span><span style={{color:"white",fontWeight:900,fontSize:22}}>{stars} Stars</span>
        </div>
      ...
      <div style={{display:"flex",gap:12,padding:"16px",background:"rgba(124,58,237,.08)",justifyContent:"center"}}>
        <button onClick={()=>{setNewName(name);setEditing(true);}} style={{background:"#fbbf24",border:"none",borderRadius:16,padding:"16px 24px",color:"#1e1b4b",cursor:"pointer",fontWeight:900,fontSize:18,boxShadow:"0 6px 16px rgba(251,191,36,.4)",display:"flex",alignItems:"center",gap:8}}>✏️<span>Edit Name</span></button>
        <button onClick={()=>setConfirmReset(true)} style={{background:"#ef4444",border:"none",borderRadius:16,padding:"16px 24px",color:"white",cursor:"pointer",fontWeight:900,fontSize:18,boxShadow:"0 6px 16px rgba(239,68,68,.4)",display:"flex",alignItems:"center",gap:8}}>🔄<span>Reset Progress</span></button>
      </div>
      ...
      <div style={{padding:"20px 16px"}}>
        ...
        {DAYS.map((day,i)=>{
          ...
          return (
            <button key={day.id} onClick={()=>!locked&&onDay(day)} style={{width:"100%",background:isDone?"#d1fae5":locked?"#f3f4f6":"white",border:`3px solid ${isDone?"#34d399":locked?"#e5e7eb":day.color}`,borderRadius:20,padding:"14px 16px",cursor:locked?"not-allowed":"pointer",display:"flex",alignItems:"center",gap:14,marginBottom:10,textAlign:"left",opacity:locked?.55:1,boxShadow:isDone?"none":"0 3px 12px rgba(0,0,0,.07)",animation:"fadein .4s ease"}}>
              <div style={{width:50,height:50,borderRadius:14,background:locked?"#d1d5db":isDone?"#34d399":day.color,display:"flex",alignItems:"center",justifyContent:"center",fontSize:24,flexShrink:0}}>{isDone?"✅":locked?"🔒":day.emoji}</div>
...
// ── DayView ───────────────────────────────────────────────────────
function DayView({day,name,isDone,onGame,onComplete,onBack}: {day: DayConfig; name: string; isDone: boolean; onGame: (g: GameContext | {type: string}) => void; onComplete: () => void; onBack: () => void}) {
  const [abcOpen,setAbcOpen]=useState<boolean>(false);
  if (abcOpen) return <AlphabetSong name={name} onBack={()=>setAbcOpen(false)}/>;
  const gameLabel: Record<string, string> = {flashcard:"🃏 Flashcard Game",hunt:"🔍 Letter Hunt",trace:"✏️ Letter Tracing",write:"✍️ Writing Practice",memory:"🎲 Memory Game"};
  return (
    <div style={{minHeight:"100vh",background:"#f0f4ff"}}>
      <div style={{background:`linear-gradient(135deg,${day.color},${day.color}bb)`,padding:"20px 16px",borderRadius:"0 0 28px 28px",textAlign:"center"}}>
      ...
      <div style={{padding:18}}>
        <div style={{background:"white",borderRadius:20,padding:18,marginBottom:14,textAlign:"center",boxShadow:"0 4px 14px rgba(0,0,0,.07)"}}>
          ...
          <div style={{display:"flex",justifyContent:"center",gap:8,flexWrap:"wrap"}}>
            {name.split("").map((l,i)=>(
              <div key={i} style={{width:50,height:50,borderRadius:12,background:`hsl(${i*47+200},65%,62%)`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:26,fontWeight:900,color:"white",boxShadow:"0 3px 8px rgba(0,0,0,.2)"}}>{l}</div>
            ))}
          </div>
        </div>
        ...
        {day.id<=2&&(
          <button onClick={()=>setAbcOpen(true)} style={{width:"100%",background:"linear-gradient(135deg,#fbbf24,#f59e0b)",border:"none",borderRadius:18,padding:18,cursor:"pointer",textAlign:"left",marginBottom:12,boxShadow:"0 4px 14px rgba(251,191,36,.4)"}}>
        ...
        <button onClick={()=>onGame({type:day.game})} style={{width:"100%",background:`linear-gradient(135deg,${day.color},${day.color}aa)`,border:"none",borderRadius:18,padding:18,cursor:"pointer",textAlign:"left",boxShadow:`0 4px 14px ${day.color}66`}}>
        ...
        {!isDone?(
          <button onClick={onComplete} style={{marginTop:16,width:"100%",padding:15,fontSize:17,fontWeight:800,background:"linear-gradient(135deg,#34d399,#10b981)",color:"white",border:"none",borderRadius:16,cursor:"pointer",boxShadow:"0 4px 14px rgba(52,211,153,.4)"}}>
            ✅ Mark Day Complete (+3 ⭐)
          </button>
        ):(
...
function AlphabetSong({name,onBack}: {name: string; onBack: () => void}) {
  ...
  const start=()=>{
    ...
    let i=0;
    ref.current=setInterval(()=>{ i++; if(i>=26){clearInterval(ref.current);setPlaying(false);}else setCur(i); },480);
  };
    ...
    <div style={{minHeight:"100vh",background:"linear-gradient(135deg,#fef3c7,#fde68a)",padding:20}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(6,1fr)",gap:7,marginBottom:22}}>
        {AZ.map((l,i)=>{
          ...
          return <div key={l} style={{aspectRatio:"1",display:"flex",alignItems:"center",justifyContent:"center",borderRadius:12,fontSize:19,fontWeight:900,transition:"all .2s",
            ...
            border:inName?"2px solid #f59e0b":"2px solid transparent"}}>{l}</div>;
        })}
      </div>
      <div style={{display:"flex",gap:12,justifyContent:"center"}}>
        <button onClick={start} disabled={playing}
          style={{padding:"13px 26px",fontSize:17,fontWeight:800,background:"#f59e0b",color:"white",border:"none",borderRadius:14,cursor:"pointer",opacity:playing?.6:1}}>
...
// ── GameRouter ────────────────────────────────────────────────────
function GameRouter({ctx,name,earn,onDone,onBack}: {ctx: GameContext | null; name: string; earn: (n: number) => Promise<void>; onDone: () => void; onBack: () => void}) {
...
// ── FlashcardGame ─────────────────────────────────────────────────
function FlashcardGame({name,earn,onDone,onBack}: {name: string; earn: (n: number) => Promise<void>; onDone: () => void; onBack: () => void}) {
  ...
  const answer=(knew: boolean): void =>{
  ...
  const l=letters[idx]; const inName=name.includes(l);
  return (
    <div style={{minHeight:"100vh",background:"linear-gradient(135deg,#fef9c3,#fef3c7)",padding:20}}>
      ...
      <div onClick={()=>setFlipped(true)} style={{margin:"0 auto 24px",maxWidth:280,height:280,borderRadius:28,background:flipped?`hsl(${l.charCodeAt(0)*11%360},65%,60%)`:"linear-gradient(135deg,#a78bfa,#ec4899)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",cursor:"pointer",boxShadow:"0 12px 40px rgba(0,0,0,.2)",transition:"all .3s"}}>
        {flipped?<><div style={{fontSize:130,fontWeight:900,color:"white",lineHeight:1}}>{l}</div><div style={{fontSize:52,color:"rgba(255,255,255,.7)"}}>{l.toLowerCase()}</div></>
...
// ── HuntGame ──────────────────────────────────────────────────────
function HuntGame({name,earn,onDone,onBack}: {name: string; earn: (n: number) => Promise<void>; onDone: () => void; onBack: () => void}) {
  ...
  const tap=(l: string, i: number): void =>{
  ...
  return (
    <div style={{minHeight:"100vh",background:"linear-gradient(135deg,#fce7f3,#fdf4ff)",padding:20}}>
      ...
      <div style={{background:"white",borderRadius:16,padding:14,marginBottom:14,boxShadow:"0 3px 10px rgba(0,0,0,.07)"}}>
        <p style={{textAlign:"center",fontWeight:700,color:"#6b7280",fontSize:13,marginBottom:8}}>Find letters in your name:</p>
        <div style={{display:"flex",justifyContent:"center",gap:8,flexWrap:"wrap"}}>
          {nameLs.map(l=>{const f=foundUniq.includes(l);return(
            <div key={l} style={{width:42,height:42,borderRadius:10,background:f?"#34d399":"#f3f4f6",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,fontWeight:900,color:f?"white":"#374151",transition:"all .25s"}}>{f?"✓":l}</div>
          );})}
        </div>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:8}}>
        {grid.map((l,i)=>{const f=found.has(i);const w=wrong===i;return(
          <button key={i} onClick={()=>tap(l,i)} style={{aspectRatio:"1",borderRadius:14,border:"none",fontSize:21,fontWeight:900,cursor:"pointer",background:f?"#34d399":w?"#f87171":"white",color:f||w?"white":"#374151",transform:w?"scale(.88)":f?"scale(1.05)":"scale(1)",transition:"all .15s",boxShadow:`0 3px 8px ${f?"rgba(52,211,153,.4)":w?"rgba(248,113,113,.4)":"rgba(0,0,0,.08)"}`}}>{f?"✓":l}</button>
        );})}
      </div>
      <p style={{textAlign:"center",marginTop:14,color:"#9d174d",fontWeight:600,fontSize:13}}>Tap each letter in: <strong>{name}</strong></p>
    </div>
  );
}

// ── TraceGame ─────────────────────────────────────────────────────
function TraceGame({name,earn,onDone,onBack}: {name: string; earn: (n: number) => Promise<void>; onDone: () => void; onBack: () => void}) {
  ...
  const getXY=(e: React.TouchEvent | React.MouseEvent, c: HTMLCanvasElement)=>{const r=c.getBoundingClientRect(),t=(e as any).touches?.[0]||(e as any);return{x:t.clientX-r.left,y:t.clientY-r.top};};  
  const guide=()=>{if(!cvs.current)return;const ctx=cvs.current.getContext("2d");if(!ctx)return;ctx.clearRect(0,0,280,280);ctx.font="bold 195px Arial";ctx.fillStyle="rgba(167,139,250,.18)";ctx.textAlign="center";ctx.textBaseline="middle";ctx.fillText(l,140,148);};
  useEffect(()=>{guide();},[idx]);

  const sd=(e: React.TouchEvent | React.MouseEvent)=>{e.preventDefault();drawing.current=true;if(!cvs.current)return;const ctx=cvs.current.getContext("2d");if(!ctx)return;const{x,y}=getXY(e,cvs.current);ctx.beginPath();ctx.moveTo(x,y);};
  const d=(e: React.TouchEvent | React.MouseEvent)=>{e.preventDefault();if(!drawing.current||!cvs.current)return;const ctx=cvs.current.getContext("2d");if(!ctx)return;const{x,y}=getXY(e,cvs.current);ctx.lineTo(x,y);ctx.strokeStyle="#a78bfa";ctx.lineWidth=9;ctx.lineCap="round";ctx.stroke();};
  ...
  return (
    <div style={{minHeight:"100vh",background:"linear-gradient(135deg,#ede9fe,#faf5ff)",padding:20}}>
      ...
      <div style={{display:"flex",justifyContent:"center",gap:8,marginBottom:16,flexWrap:"wrap"}}>
        {letters.map((x,i)=><div key={i} style={{width:44,height:44,borderRadius:10,background:i<idx?"#a78bfa":i===idx?"#7c3aed":"#e5e7eb",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,fontWeight:900,color:i<=idx?"white":"#9ca3af",border:i===idx?"3px solid #6d28d9":"3px solid transparent"}}>{x}</div>)}
      </div>
      ...
      <div style={{display:"flex",justifyContent:"center",marginBottom:16}}>
        <canvas ref={cvs} width={280} height={280} style={{borderRadius:24,background:"white",boxShadow:"0 8px 28px rgba(124,58,237,.2)",border:"3px solid #c4b5fd",cursor:"crosshair",touchAction:"none"}}
      ...
      <div style={{display:"flex",gap:12,justifyContent:"center"}}>
        ...
        <button onClick={next} style={{padding:"12px 26px",fontSize:15,fontWeight:800,background:"linear-gradient(135deg,#a78bfa,#7c3aed)",color:"white",border:"none",borderRadius:14,cursor:"pointer"}}>{idx<letters.length-1?"Next Letter →":"All Done! 🎉"}</button>
      </div>
    </div>
  );
}
...
// ── WriteGame ─────────────────────────────────────────────────────
function WriteGame({name,earn,onDone,onBack}: {name: string; earn: (n: number) => Promise<void>; onDone: () => void; onBack: () => void}) {
  ...
  const check=(): void =>{
    ...
    if(v===name){setResult("ok");earn(2);}
    else setResult("no");
  };
  if(result==="ok") return <Win score={2} total={2} msg={`You wrote ${name} perfectly!`} onDone={onDone}/>;
  return (
    <div style={{minHeight:"100vh",background:"linear-gradient(135deg,#e0f2fe,#f0f9ff)",padding:20}}>
      ...
      <div style={{background:"white",borderRadius:20,padding:18,marginBottom:14,boxShadow:"0 4px 14px rgba(0,0,0,.07)"}}>
        ...
        <input value={input} onChange={(e: React.ChangeEvent<HTMLInputElement>)=>{setInput(e.target.value);setResult(null);}} maxLength={20}
      ...
      <div style={{background:"#e0f2fe",borderRadius:14,padding:12,marginBottom:14}}>
        <p style={{textAlign:"center",color:"#0c4a6e",fontWeight:600,margin:0,fontSize:13}}>💡 Say each letter aloud: {name.split("").join(" — ")}</p>
      </div>
      <div style={{display:"flex",gap:12,justifyContent:"center"}}>
...
// ── MemoryGame ────────────────────────────────────────────────────
function MemoryGame({name,earn,onDone,onBack}: {name: string; earn: (n: number) => Promise<void>; onDone: () => void; onBack: () => void}) {
  interface Tile { l: string; id: number; }
  interface GameState { tiles: Tile[]; placed: (Tile | null)[]; sel: Tile | null; moves: number; }
  const init=(): GameState =>({tiles:shuffle(name.split("").map((l,i)=>({l,id:i}))),placed:Array(name.length).fill(null),sel:null,moves:0});
  ...
  const tapTile=(t: Tile): void =>setState(s=>({...s,sel:s.sel?.id===t.id?null:t}));
  const tapSlot=(i: number): void =>{
    if(!sel) return;
    if(placed[i]) return;
    const np=[...placed]; np[i]=sel;
    const newMoves=moves+1;
    const won=np.every((p): p is Tile => p !== null)&&np.map(p=>p.l).join("")===name;
    setState(s=>({...s,placed:np,sel:null,moves:newMoves}));
    if(won){earn(3);setTimeout(()=>onDone(),400);}
  };
  const removeSlot=(i: number): void =>setState(s=>{const np=[...s.placed];np[i]=null;return{...s,placed:np};});
  ...
  return (
    <div style={{minHeight:"100vh",background:"linear-gradient(135deg,#d1fae5,#ecfdf5)",padding:20}}>
      ...
      <p style={{textAlign:"center",color:"#065f46",fontWeight:600,marginBottom:16,fontSize:14}}>
        {sel?`Tap a blank slot to place "${sel.l}"`:`Tap a letter tile, then tap a slot. Spell: `}<strong>{!sel&&name}</strong>
      </p>
      ...
      <div style={{display:"flex",justifyContent:"center",gap:8,marginBottom:20,flexWrap:"wrap"}}>
        {placed.map((tile,i)=>(
          <div key={i} onClick={()=>tile?removeSlot(i):tapSlot(i)}
            ...
            {tile?<><span style={{fontSize:24,fontWeight:900,color:"white"}}>{tile.l}</span><span style={{fontSize:10,color:"rgba(255,255,255,.75)"}}>remove</span></>:<span style={{color:"#86efac",fontSize:20}}>+</span>}
          </div>
        ))}
      </div>
      {/* Tiles */}
      <div style={{display:"grid",gridTemplateColumns:`repeat(${Math.min(name.length,6)},1fr)`,gap:10,maxWidth:360,margin:"0 auto"}}>
        {available.map(t=>{const s2=sel?.id===t.id;return(
          <button key={t.id} onClick={()=>tapTile(t)} style={{aspectRatio:"1",borderRadius:14,border:`3px solid ${s2?"#10b981":"transparent"}`,fontSize:26,fontWeight:900,background:s2?"#a7f3d0":"white",color:s2?"#065f46":"#374151",cursor:"pointer",transform:s2?"scale(1.12)":"scale(1)",transition:"all .15s",boxShadow:`0 4px 12px ${s2?"rgba(16,185,129,.4)":"rgba(0,0,0,.08)"}`}}>{t.l}</button>
        );})}
      </div>
